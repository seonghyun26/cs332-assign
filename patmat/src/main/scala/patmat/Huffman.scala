...
object Huffman {
  ...
  def decode(tree: CodeTree, bits: List[Bit]): List[Char] = {
    def decodeRec(subtree: CodeTree, bits: List[Bit]): List[Char] = {
      subtree match {
        case Leaf(char, _) => {
        ...
        case Fork(left, right, _, _) => {
          if ( bits.head == 0 ) decodeRec(left, bits.tail)
          else decodeRec(right, bits.tail)
        }
      }
    }
    decodeRec(tree, bits)
  ...
  def codeBits(table: CodeTable)(char: Char): List[Bit] = {
    if (table == Nil ) Nil
    else table.filter( code => code._1 == char ).head._2
  }
  ...
  def mergeCodeTables(a: CodeTable, b: CodeTable): CodeTable = {
    def addBit(b: Bit)(code: (Char, List[Bit])): (Char, List[Bit]) = (code._1, b :: code._2)
  ...
  def quickEncode(tree: CodeTree)(text: List[Char]): List[Bit] = {
    if (text == Nil) Nil
    else text flatMap codeBits(convert(tree))
  }
}
