...
trait NodeScala {
  ...
  private def respond(exchange: Exchange, token: CancellationToken, response: Response): Unit = {
    ...
    {
      val nextResponse = response.next()
      exchange.write(nextResponse)
    }
    exchange.close()
  }
  ...
  def start(relativePath: String)(handler: Request => Response): Subscription = {
    ...
    val cancelSubscription = Future.run() { cancellationToken =>
      async {
    ...
    Subscription(listenerSubscription, cancelSubscription)
  }
