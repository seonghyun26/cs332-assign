...
trait Solver extends GameDef {
  ...
  def from(
    ...
    if (initial.isEmpty)  Stream.empty
      ...
      val exploredNew = neighborsNew.map(_._1)
      initial.head #:: from(initial.tail #::: neighborsNew, explored ++ exploredNew)
    }
  }
   ...
   */
  lazy val pathsToGoal: Stream[(Block, List[Move])] = pathsFromStart filter {blocknMove => done(blocknMove._1)}
  ...
  lazy val solution: List[Move] = {
    ...
    else pathsToGoal.map{_._2.reverse}.head
  }
}
