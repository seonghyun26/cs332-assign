...
object Anagrams {
  ...
  def combinations(occurrences: Occurrences): List[Occurrences] = occurrences match {
    ...
    case head :: tail => { for {
        ...
        headCnt <- 0 to head._2
      } yield {
        if (headCnt == 0) tailList
        else (head._1, headCnt) :: tailList
      }
    } 
  }
  ...
  def sentenceAnagrams(sentence: Sentence): List[Sentence] = {
    def sentenceAnagramsRec(occurrences: Occurrences): List[Sentence] = occurrences match{
      // println(occurrences)
      case Nil => List(Nil)
      case _ => {
        for {
          ...
          sentence <- sentenceAnagramsRec(subtract(occurrences, combination))
        } yield word :: sentence
      }
    }
